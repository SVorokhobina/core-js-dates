...
function getDayName(date) {
  ...
  return dayArr[currentDate.getUTCDay()];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const lastDay = new Date(dateEnd);
  lastDay.setDate(lastDay.getDate() + 1);
  return (lastDay - firstDay) / (1000 * 3600 * 24);
}
...
function getWeekNumberByDate(date) {
  ...
  const weekStart = new Date(date.getUTCFullYear(), 0, 1);
  const weekStartTS = weekStart.getTime();
  const weekStartDOW = weekStart.getDay();
  const msPerDay = 86400000;
  let weekEndTS =
    weekStartDOW === 0
      ? weekStartTS + msPerDay
      : weekStartTS + (7 - weekStartDOW + 1) * msPerDay;
  let counter = 1;
  ...
  while (dateTS >= weekEndTS) {
    weekEndTS += msPerDay * 7;
    counter += 1;
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const result = [];
  const periodLength = (endDate - currentDate) / (1000 * 3600 * 24) + 1;

  let n = 0;
  while (n < periodLength) {
    for (let k = 0; k < countWorkDays; k += 1) {
      const strDate = String(currentDate.getDate()).padStart(2, 0);
      const strMonth = String(currentDate.getMonth() + 1).padStart(2, 0);
      const strYear = currentDate.getFullYear();
      const str = `${strDate}-${strMonth}-${strYear}`;
      result.push(str);
